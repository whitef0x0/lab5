...
In this lab activity, you will also have to perform a `git merge` operation between two differing repository versions.
...
You will now have the situation that the local repository and the remote repository (on BitBucket) are divergent. Use the `merge` feature of `git` to reconcile differences. You should read the section on [merging](http://wiki.eclipse.org/EGit/User_Guide#Merging) that is part of the EGit user guide and consult your teaching assistant for help as needed.
